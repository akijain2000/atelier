...
export function verifyWebhookSignature(secret) {
  return async (req, res, next) => {
    ...
    if (sigBuffer.length === expectedBuffer.length) {
      try { hmacMatch = crypto.timingSafeEqual(sigBuffer, expectedBuffer); } catch {}
    }

    if (!hmacMatch) {
      const reserializedExpected = crypto
        .createHmac('sha256', secret)
        .update(JSON.stringify(req.body))
        .digest('hex');
      const reserializedBuffer = Buffer.from(reserializedExpected, 'hex');
      if (sigBuffer.length === reserializedBuffer.length) {
        try { hmacMatch = crypto.timingSafeEqual(sigBuffer, reserializedBuffer); } catch {}
      }
    }

    if (!hmacMatch) {
      const tokenHeader = req.headers['x-webhook-token'];
      if (tokenHeader && tokenHeader.length === secret.length) {
        try {
          const tokenBuf = Buffer.from(tokenHeader);
          const secretBuf = Buffer.from(secret);
          hmacMatch = crypto.timingSafeEqual(tokenBuf, secretBuf);
        } catch {}
      }
    }
