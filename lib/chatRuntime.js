...
async function sendAnthropicChat({ model, systemPrompt, messages, max_tokens }) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    const body = {
      model,
      max_tokens: max_tokens || anthropicMaxTokens,
      system: [
        {
          type: 'text',
          text: systemPrompt,
...
export async function sendChat({ provider, model = null, systemPrompt, messages, max_tokens }) {
